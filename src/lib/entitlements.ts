...
// Beta grandfathering (former Terms §4B "Lifetime Standard Tier Safeguard") was
// removed on 2026-08-29. No accounts registered during the beta window, so the
// grandfather granted a tier to nobody; the promise was retracted and accounts
// now resolve to their purchased tier (or `free`) like any other.
...
export async function resolveEntitlements(req: Request): Promise<LicenseClaim | null> {
  ...
  if (!rows.length) {
    // Authenticated but no entitlements → free tier, no products.
    return {
      license_id: user.id,
      tier: "free",
      products: [],
